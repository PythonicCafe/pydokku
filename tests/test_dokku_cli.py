...
# TODO: implement tests for Dokku.apps_* methods
# TODO: implement tests for Dokku.config_* using an app (instead of global), alternating between restart=True|False
...
def test_config_set_get():
    ...
    dokku = Dokku()
    configs_before = dokku.config_get(app_name=None)
    dokku.config_set(app_name=None, key=key1, value=value1)
    configs_after = dokku.config_get(app_name=None)
    ...
    dokku.config_unset(app_name=None, key=key1)
    new_configs = dokku.config_get(app_name=None)
    del expected[key1]
    assert len(expected) == len(new_configs)
    pairs = {key1: str(value1 or ""), key2: str(value2 or ""), key3: str(value3 or ""), key4: str(value4 or "")}
    dokku.config_set_many(app_name=None, keys_values=pairs)
    final_configs = dokku.config_get(app_name=None)
    expected = configs_before.copy()
...
def test_config_clear():
    ...
    dokku.config_clear(app_name=None)
    final_configs = dokku.config_get(app_name=None)
    assert len(final_configs) == 0

def test_ssh_keys():
    dokku = Dokku()
    keys = dokku.ssh_keys_list()
    assert len(keys) == 0
    ...
    result = dokku.ssh_keys_add(name=name, content=content)
