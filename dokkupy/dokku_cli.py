...
class Dokku:
    ...
    def apps(self) -> list[str]:
        _, stdout, stderr = self._execute(["dokku", "apps:report"], check=False)
        if not stdout and "You haven't deployed any applications yet" in stderr:
            return []
        apps_infos = re.split("^=====> ", stdout, flags=re.MULTILINE)[1:]
        result = []
    ...
    def apps_list(self) -> list[str]:
        _, stdout, _ = self._execute(["dokku", "apps:list"])
        return stdout.splitlines()[1:]

    def apps_create(self, name: str) -> str:
        _, stdout, _ = self._execute(["dokku", "apps:create", name])
        return stdout

    def apps_destroy(self, name: str) -> str:
        _, stdout, _ = self._execute(["dokku", "apps:destroy", name], stdin=name)
        return stdout

    def apps_clone(self, old_name: str, new_name: str) -> str:
        _, stdout, _ = self._execute(["dokku", "apps:clone", old_name, new_name])
        return stdout

    def apps_lock(self, name: str) -> str:
        _, stdout, _ = self._execute(["dokku", "apps:lock", name])
        return stdout

    def apps_unlock(self, name: str) -> str:
        _, stdout, _ = self._execute(["dokku", "apps:unlock", name])
        return stdout

    def apps_locked(self, name: str) -> str:
        _, stdout, stderr = self._execute(["dokku", "apps:unlock", name], check=False)
        return bool(stdout)

    def apps_rename(self, old_name: str, new_name: str) -> str:
        _, stdout, _ = self._execute(["dokku", "apps:rename", old_name, new_name])
        return stdout

    def config_get(self, app_name: str) -> str:
        ...
        cmd = ["dokku", "config:export", "--format=json"]
        cmd.append("--global" if system else app_name)
        _, stdout, _ = self._execute(cmd)
        return json.loads(stdout)

    def config_set_many(self, app_name: str, keys_values: dict, restart: bool = False) -> str:
        system = app_name is None
        ...
        cmd = ["dokku", "config:set", "--encoded"]
        if not restart and not system:
            cmd.append("--no-restart")
        cmd.append("--global" if system else app_name)
        _, stdout, _ = self._execute(cmd + [f"{key}={value}" for key, value in encoded_pairs.items()])
        return stdout

    def config_set(self, app_name: str, key: str, value: str, restart: bool = False) -> str:
        return self.config_set_many(app_name=app_name, keys_values={key: value}, restart=restart)

    def config_unset_many(self, app_name: str, keys: list[str], restart: bool = False) -> str:
        system = app_name is None
        if system and restart:
            raise ValueError("Cannot restart when setting global config")
        cmd = ["dokku", "config:unset"]
        if not restart and not system:
            cmd.append("--no-restart")
        cmd.append("--global" if system else app_name)
        _, stdout, _ = self._execute(cmd + keys)
        return stdout

    def config_unset(self, app_name: str, key: str, restart: bool = False) -> str:
        return self.config_unset_many(app_name=app_name, keys=[key], restart=restart)

    def config_clear(self, app_name: str, restart: bool = False) -> str:
        system = app_name is None
        if system and restart:
            raise ValueError("Cannot restart when setting global config")
        cmd = ["dokku", "config:clear"]
        if not restart and not system:
            cmd.append("--no-restart")
        cmd.append("--global" if system else app_name)
        _, stdout, _ = self._execute(cmd)
    ...
    def ssh_keys_list(self) -> list[dict]:
        ...
        # there's no way to create a ssh key with these options
        # For some weird reason, using `dokku ssh-keys:list --format=json` outputs nothing even when there are keys
        _, stdout, stderr = self._execute(["dokku", "ssh-keys:list"], check=False)
    ...
    # TODO: ssh-keys:add <name> [/path/to/key]                   Add a new public key by pipe or path
    # TODO: ssh-keys:remove [--fingerprint fingerprint|<name>]   Remove SSH public key by name

    # TODO: storage:ensure-directory [--chown option] <directory>   Creates a persistent storage directory in the recommended storage path
    # TODO: storage:list <app> [--format text|json]                 List bind mounts for app's container(s) (host:container)
    # TODO: storage:mount <app> <host-dir:container-dir>            Create a new bind mount
    # TODO: storage:report [<app>] [<flag>]                         Displays a checks report for one or more apps
    # TODO: storage:unmount <app> <host-dir:container-dir>          Remove an existing bind mount
